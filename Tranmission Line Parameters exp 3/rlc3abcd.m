...
function [Z, Y, ABCD] = rlc3abcd(r, L, C, g, f, Length);
...
if g== 0 & C==0
   ...
   model = input('Enter 1 for Medium line(Nominal Pi Model) or 2 for long line or 3 for 1 for Medium line(Nominal T model) --> ');
    if model ~=1 & model~=2 & model~=3
    disp(' Enter 1 or 2 or 3'), else, end
end
  ...
  if model==2
   ...
   if model==1,  disp(' '),  disp(' Nominal pi model'),
      ...
      fprintf('   = %gø\n', imag(Gamal*180/pi))
      elseif model==3,  disp(' '),  disp(' Nominal T model'),
      disp(' ----------------')
      fprintf('\n'), fprintf(' Z = %g + j ',R), fprintf('%g ohms\n',X),
      fprintf(' Y = %g + j ',real(Y)), fprintf('%g Siemens\n',imag(Y)) 
   end,%end
...
if model ==-1 |model ==1 |model ==2
  ...
  fprintf(' \n')
  elseif model ==-1 |model ==3
  A = 1 + Z*Y/2;  B = Z*(1 + Z*Y/4);
  C = Y;  D = A;
  ABCD = [A B; C D];
  disp(' ')
  fprintf('         %-11.5g + j', real(A)), fprintf(' %-11.5g', imag(A))
  ...
  fprintf('   %-11.5g + j', real(D)), fprintf(' %-11.5g  \n', imag(D))
  fprintf(' \n')
else, fprintf(' You must enter 1 or 2 or 3. Try again \n'), return, end
